...
public class AuthServiceTest {
    ...
    void expiredRefreshTokenTest() {
        ...
        assertThatThrownBy(() -> authService.reissueToken("expired-refresh-token"))
                .isInstanceOfSatisfying(CustomException.class, exception ->
    ...
    void reissueTokenSuccessTest() {
        ...
        when(tsidGenerator.nextId())
                .thenReturn(200L);
        when(jwtProvider.createRefreshToken(10L, 200L))
                .thenReturn("new-refresh-token");
        when(tokenHasher.hash("new-refresh-token"))
        ...
        TokenPair result = authService.reissueToken("valid-refresh-token");
        ...
        assertThat(refreshTokenCaptor.getValue().getId()).isEqualTo(200L);
        assertThat(refreshTokenCaptor.getValue().getUserId()).isEqualTo(10L);
        assertThat(refreshTokenCaptor.getValue().getTokenHash()).isEqualTo("new-refresh-token-hash");
        assertThat(ReflectionTestUtils.getField(result, "accessToken")).isEqualTo("new-access-token");
        assertThat(ReflectionTestUtils.getField(result, "refreshToken")).isEqualTo("new-refresh-token");
    }
}
