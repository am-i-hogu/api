...
public class MyCommentQueryServiceTest {

    private final CursorCodec cursorCodec = mock(CursorCodec.class);
    private final CommentRepository commentRepository = mock(CommentRepository.class);
    private final MyCommentQueryService myCommentQueryService = new MyCommentQueryService(
            cursorCodec, commentRepository
    );
     ...
     */
    @Test
    void getMyCommentsThrowsInvalidParamValueWhenCursorIsInvalid() {
        when(cursorCodec.decode("invalid-cursor", MyCommentCursor.class))
                .thenThrow(new IllegalStateException("Failed to decode cursor"));

        assertThatThrownBy(() -> myCommentQueryService.getMyComments(
                1L,
