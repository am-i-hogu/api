...
public class MyVoteQueryServiceTest {
     ...
     */
    @Test
    void getMyVotesThrowsInvalidParamValueWhenCursorIsInvalid() {
        when(cursorCodec.decode("invalid-cursor", MyVoteCursor.class))
                .thenThrow(new IllegalStateException("Failed to decode cursor"));

        assertThatThrownBy(() -> myVoteQueryService.getMyVotes(
                1L,
